...
# This program is for scraping PDF data from individual states 
# based on documents downloaded from the internet
...
for (i in 1:nrow(DE_df_combined)){
  ...
  name <- row$`AUG_School District`
  matches <- grep('FPD', row, value=TRUE)
  # date <- names(matches[length(matches)])
  if (length(date) == 0) {
    first_dates <- append(first_dates, list(name, NA, NA, NA))
  } else {
    matches <- c(name, matches, rep(NA, 3 - length(matches)))
    first_dates <- append(first_dates, list(matches))
  }
}
matrix(unlist(first_dates), nrow=4)
data.frame(first_dates)
